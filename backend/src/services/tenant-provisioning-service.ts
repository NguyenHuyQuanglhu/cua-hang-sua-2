...
export class TenantProvisioningService {
   ...
   */
  private updateProgress(tenantId: string, status: ProvisioningStatus, progress: number, message: string, error?: string): void {
   ...
   */
  private async createDatabase(databaseName: string, databaseServer: string): Promise<void> {
   ...
   * Wait for database to be ready
   */
  private async waitForDatabase(databaseName: string, databaseServer: string, maxAttempts = 10): Promise<void> {
    const masterPool = tenantRouter.getMasterConnection();
   ...
   */
  private async runMigrations(databaseName: string, databaseServer: string): Promise<void> {
   ...
   */
  private async createTenantTables(pool: sql.ConnectionPool): Promise<void> {
   ...
   */
  private async createMoreTenantTables(pool: sql.ConnectionPool): Promise<void> {
    // 8. Units table
    await pool.request().query(`
   ...
   */
  private async createRemainingTenantTables(pool: sql.ConnectionPool): Promise<void> {
   ...
   * Create indexes for performance
   */
  private async createIndexes(pool: sql.ConnectionPool): Promise<void> {
    const indexes = [
  ...
  private async createDefaultStore(
    ...
    try {
      ...
      await pool.request()
        .input('id', sql.NVarChar, settingsId)
   ...
   */
  private async assignOwnerToStore(
    databaseName: string,
    databaseServer: string,
    ownerId: string,
    storeId: string
    ...
    try {
      ...
      await pool.request()
        .input('id', sql.NVarChar, userStoreId)
        .input('userId', sql.NVarChar, ownerId)
        .input('storeId', sql.NVarChar, storeId)
        .query(`
   ...
   */
  private async cleanupFailedProvisioning(databaseName: string, databaseServer: string): Promise<void> {
    try {
      const masterPool = tenantRouter.getMasterConnection();
