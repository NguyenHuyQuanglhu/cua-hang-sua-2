...
class StatusMapper {
     ...
     */
    static isValidNewStatus(status) {
    ...
    static normalize(status) {
        ...
        if (this.isValidOldStatus(status)) {
            return this.mapOldToNew(status);
        }
        // Invalid status value
        throw new Error(`Invalid status value: "${status}". Must be one of: pending, processed, draft, printed, completed, cancelled`);
    }
    /**
     ...
     */
    static isValidOldStatus(status) {
        return status === 'draft' ||
            status === 'printed' ||
            status === 'completed' ||
            status === 'cancelled';
