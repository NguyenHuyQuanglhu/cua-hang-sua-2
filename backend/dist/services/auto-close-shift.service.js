...
exports.autoCloseShiftService = exports.AutoCloseShiftService = void 0;
const node_cron_1 = __importDefault(require("node-cron"));
const db_1 = require("../db");
/**
 * Service tự động đóng ca làm việc khi hết thời gian
 */
class AutoCloseShiftService {
    cronJob = null;
    isRunning = false;
    /**
     ...
     */
    async checkAndCloseExpiredShifts() {
        try {
            // Lấy tất cả ca đang active
            const activeShifts = await (0, db_1.query)(`SELECT s.*, u.max_shift_hours
         FROM Shifts s
            ...
            for (const shift of activeShifts) {
                ...
                if (hoursWorked >= maxShiftHours) {
                    try {
                        ...
                        closedCount++;
                        console.log(`✅ Đã tự động đóng ca cho ${shift.user_name} (${hoursWorked.toFixed(2)}/${maxShiftHours} giờ)`);
                    }
                    catch (error) {
     ...
     * Tự động đóng ca
     */
    async autoCloseShift(shiftId, storeId, startingCash) {
       ...
       WHERE shift_id = @shiftId AND store_id = @storeId`, { shiftId, storeId });
        const shiftRecord = await (0, db_1.queryOne)(`SELECT start_time FROM Shifts WHERE id = @shiftId`, { shiftId });
        const paymentsSummary = await (0, db_1.queryOne)(`SELECT ISNULL(SUM(amount), 0) as cash_payments
       FROM Payments 
       WHERE store_id = @storeId 
         AND payment_date >= @startTime 
         AND payment_date <= GETDATE()`, { storeId, startTime: shiftRecord?.start_time });
...
exports.AutoCloseShiftService = AutoCloseShiftService;
// Export singleton instance
exports.autoCloseShiftService = new AutoCloseShiftService();
//# sourceMappingURL=auto-close-shift.service.js.map