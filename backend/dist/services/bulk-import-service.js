...
exports.parseCustomersExcel = parseCustomersExcel;
exports.importCustomers = importCustomers;
exports.generateCustomerImportTemplate = generateCustomerImportTemplate;
...
async function importProducts(products, storeId, tenantId) {
    ...
    const categoriesResult = await (0, db_1.query)(`SELECT CategoryID, CategoryName FROM Categories WHERE TenantID = @tenantId`, { tenantId });
    const unitsResult = await (0, db_1.query)(`SELECT UnitID, UnitName FROM Units WHERE TenantID = @tenantId`, { tenantId });
    ...
    for (let i = 0; i < products.length; i++) {
        ...
        try {
            ...
            if (product.barcode) {
                const existingResult = await (0, db_1.query)(`SELECT ProductID FROM Products WHERE Barcode = @barcode AND TenantID = @tenantId`, { barcode: product.barcode, tenantId });
...
async function exportProducts(options) {
  ...
  `;
    const result = await (0, db_1.query)(sqlQuery, { storeId, tenantId });
...
function parseCustomersExcel(buffer) {
    const workbook = XLSX.read(buffer, { type: 'buffer' });
    const sheetName = workbook.SheetNames[0];
    const worksheet = workbook.Sheets[sheetName];
    const data = XLSX.utils.sheet_to_json(worksheet);
...
async function importCustomers(customers, storeId) {
    const result = {
        success: true,
    ...
    for (let i = 0; i < customers.length; i++) {
        ...
        try {
            ...
            if (customer.phone) {
                const existing = await (0, db_1.query)(`SELECT id FROM Customers WHERE phone = @phone AND store_id = @storeId`, { phone: customer.phone, storeId });
             ...
             WHERE phone = @phone AND store_id = @storeId`, {
                    ...
                    // Insert new customer
                    await (0, db_1.query)(`INSERT INTO Customers (
...
function generateCustomerImportTemplate() {
    const templateData = [
        {
            'Tên': 'Nguyễn Văn A',
...
async function exportCustomers(storeId) {
