...
async function logNotification(data) {
    try {
        await (0, index_1.query)(`INSERT INTO NotificationLogs (Type, Channel, Recipient, Subject, Status, Error, CreatedAt)
...
async function checkAndSendLowStockAlerts(storeId, tenantId, alertEmail) {
    try {
        const result = await (0, index_1.query)(`SELECT p.ProductName, ISNULL(pi.Quantity, 0) as CurrentStock, p.MinStockLevel
       FROM Products p
       ...
       WHERE p.TenantID = @tenantId AND p.IsActive = 1
        ...
        // Get store name
        const storeResult = await (0, index_1.query)(`SELECT StoreName FROM Stores WHERE StoreID = @storeId`, { storeId });
