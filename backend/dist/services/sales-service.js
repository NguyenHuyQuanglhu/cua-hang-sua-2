...
class SalesService {
     ...
     */
    async createSale(saleData, storeId, userId) {
        return (0, transaction_1.withTransaction)(async (transaction) => {
            ...
            for (const [key, totalQty] of requestedQuantities.entries()) {
                ...
                if (available < totalQty) {
                    ...
                    throw new inventory_service_1.InsufficientStockError(`Không đủ hàng. Chỉ còn ${available} ${product?.name || 'sản phẩm'}`, productId, totalQty, available, unitId);
                }
            }
            // Calculate totals
         ...
         VALUES (@id, @storeId, @invoiceNumber, @customerId, @shiftId, @transactionDate,
                ...
                customerPayment,
                previousDebt,
                contractorId: saleData.contractorId || null,
           ...
           VALUES (@id, @salesTransactionId, @productId, @quantity, @price, @unitId, @createdAt)`, {
            ...
            if (saleData.customerId) {
             ...
             WHERE id = @customerId AND store_id = @storeId`, {
                ...
                if (excessPayment > 0) {
                    const salesWithDebt = await (0, db_1.query)(`SELECT id, remaining_debt FROM Sales
             WHERE customer_id = @customerId AND store_id = @storeId AND remaining_debt > 0
             ORDER BY transaction_date ASC, created_at ASC`, { customerId: saleData.customerId, storeId });
                    ...
                    for (const oldSale of salesWithDebt) {
                        ...
                        await (0, db_1.query)(`UPDATE Sales SET remaining_debt = @newDebt, updated_at = @updatedAt WHERE id = @saleId`, { newDebt: newOldSaleDebt, updatedAt: now, saleId: oldSale.id });
    ...
    mapSaleToEntity(record) {
        return {
            ...
            contractorId: record.contractor_id || record.ContractorId || undefined,
