...
router.get('/customers/template', auth_1.authenticate, async (req, res) => {
    try {
...
router.get('/customers/export', auth_1.authenticate, auth_1.storeContext, async (req, res) => {
    try {
        const storeId = req.storeId;
        if (!storeId) {
            return res.status(400).json({ error: 'Store context required' });
...
router.post('/customers/import', auth_1.authenticate, auth_1.storeContext, upload.single('file'), async (req, res) => {
    try {
        const storeId = req.storeId;
        if (!storeId) {
            return res.status(400).json({ error: 'Store context required' });
