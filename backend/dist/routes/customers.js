    ...
    try {
        ...
        ORDER BY
          ...
          CASE
            WHEN ISNULL(ps.transaction_count, 0) > 0 THEN ISNULL(ps.transaction_count, 0)
            ELSE ISNULL(ss.sale_count, 0)
          END DESC`, { customerId: id, storeId });
        res.json({
            success: true,
            data: rows.map((row) => ({
...
router.put('/:id/discounts/:discountId', (0, validate_uuid_1.validateUUID)(), async (req, res) => {
    try {
        ...
        await ensureCustomerDiscountTables();
        const existing = await (0, db_1.queryOne)(`SELECT TOP 1 project_name
       FROM CustomerDiscountTransactions
       ...
       SELECT @@ROWCOUNT AS affected;`, {
        ...
        if (oldProjectName && newProjectName && oldProjectName.toLowerCase() !== newProjectName.toLowerCase()) {
            ...
            const legacyAliasRows = await (0, db_1.query)(`SELECT DISTINCT LTRIM(RTRIM(s.project_name)) AS legacy_project_name
         FROM CustomerDiscountTransactions cdt
         INNER JOIN Sales s
           ON cdt.source_sale_id = s.id
         ...
         WHERE cdt.customer_id = @customerId
           ...
           AND LTRIM(RTRIM(s.project_name)) <> @oldProjectName`, {
            ...
            const legacyAliases = Array.from(new Set(legacyAliasRows
