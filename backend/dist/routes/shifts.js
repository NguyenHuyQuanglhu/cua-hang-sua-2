...
router.get('/', async (req, res) => {
    try {
        ...
        const shifts = await (0, db_1.query)(`SELECT
         s.*,
       ...
       ORDER BY s.start_time DESC`, { storeId });
...
router.get('/active', async (req, res) => {
    try {
        ...
        const shift = await (0, db_1.queryOne)(`SELECT s.*, u.hourly_rate, u.max_shift_hours
       FROM Shifts s
       LEFT JOIN Users u ON s.user_id = u.Id
       WHERE s.store_id = @storeId AND s.user_id = @userId AND s.status = 'active'
       ORDER BY s.start_time DESC`, { storeId, userId });
        if (!shift) {
            res.json(null);
            return;
        }
        ...
        COUNT(*) as salesCount
       FROM Sales
       WHERE store_id = @storeId AND shift_id = @shiftId`, { storeId, shiftId: shift.id });
        const paymentsResult = await (0, db_1.queryOne)(`SELECT ISNULL(SUM(amount), 0) as total
       FROM Payments
       WHERE store_id = @storeId AND payment_date >= @startTime`, { storeId, startTime: shift.start_time });
...
router.post('/start', async (req, res) => {
    try {
        ...
        const existingShift = await (0, db_1.queryOne)(`SELECT id FROM Shifts WHERE store_id = @storeId AND user_id = @userId AND status = 'active'`, { storeId, userId });
        if (existingShift) {
       ...
       OUTPUT INSERTED.*
       VALUES (NEWID(), @storeId, @userId, @userName, 'active', @startTime, @startingCash, @createdAt, @updatedAt)`, { storeId, userId, userName, startTime: now, startingCash, createdAt: now, updatedAt: now });
...
router.post('/cancel-overtime-request', async (req, res) => {
    try {
        const storeId = req.storeId;
        const userId = req.user.id;
        ...
        const shift = await (0, db_1.queryOne)('SELECT id, user_id, status FROM Shifts WHERE id = @shiftId AND store_id = @storeId', { shiftId, storeId });
        if (!shift) {
            res.status(404).json({ error: 'Shift not found' });
            return;
        }
        ...
        const managers = await (0, db_1.query)(`SELECT u.id, u.email, u.display_name 
       FROM Users u
       ...
       AND u.status = 'active'`, {});
        ...
        for (const manager of managers) {
            await (0, db_1.query)(`INSERT INTO Notifications (id, user_id, type, title, message, related_id, created_at, is_read)
        ...
        // Log the request
        await (0, db_1.query)(`INSERT INTO AuditLogs (id, store_id, user_id, action, entity_type, entity_id, details, created_at)
