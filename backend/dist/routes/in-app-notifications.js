...
router.get('/', async (req, res) => {
    try {
        ...
        request.input('storeId', mssql_1.default.UniqueIdentifier, storeId);
        request.input('userId', mssql_1.default.UniqueIdentifier, userId || null);
        if (unreadOnly === 'true') {
            whereClause += ' AND is_read = 0';
        }
        if (type) {
            whereClause += ' AND type = @type';
            request.input('type', mssql_1.default.NVarChar(50), type);
        }
        // Get total count
    ...
    `);
        ...
        request.input('pageSize', mssql_1.default.Int, pageSizeNum);
...
router.post('/', async (req, res) => {
    try {
        const storeId = req.storeId;
        const { userId, type, title, message, data, priority = 'normal', actionUrl, expiresAt, } = req.body;
        ...
        await pool
            ...
            .input('title', mssql_1.default.NVarChar(255), title)
            .input('message', mssql_1.default.NVarChar(mssql_1.default.MAX), message)
            .input('data', mssql_1.default.NVarChar(mssql_1.default.MAX), data ? JSON.stringify(data) : null)
            .input('priority', mssql_1.default.NVarChar(20), priority)
            .input('actionUrl', mssql_1.default.NVarChar(500), actionUrl || null)
            .input('expiresAt', mssql_1.default.DateTime, expiresAt || null)
            .query(`
