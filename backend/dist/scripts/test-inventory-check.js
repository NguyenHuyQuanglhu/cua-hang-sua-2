...
async function testInventoryCheck() {
    try {
      ...
      `);
        ...
        const product = products.recordset.find((p) => p.name.includes('Sữa'));
        if (!product) {
        ...
        const units = await pool.request()
            .input('storeId', mssql_1.default.NVarChar, product.store_id)
            .query(`
        SELECT 
          id,
          name,
      ...
      `);
        ...
        inventory.recordset.forEach((inv) => {
        ...
        for (const unit of units.recordset.slice(0, 3)) {
            const result = await pool.request()
                .input('productId', mssql_1.default.NVarChar, product.id)
                .input('storeId', mssql_1.default.NVarChar, product.store_id)
                .input('unitId', mssql_1.default.NVarChar, unit.id)
...
testInventoryCheck();
//# sourceMappingURL=test-inventory-check.js.map